...
big_building_image = Image.open("paatest/building.png")
medium_building_image = Image.open("paatest/building.png")
small_building_image = Image.open("paatest/building.png")
house_image = Image.open("paatest/building.png")
...
def generate_map(map_size, cell_size):
    ...
    def place_building(image, x, y, width, height):
        ...
        canvas.paste(building, (x * cell_size, y * cell_size), building)
    ...
    def generate_roads():
        ...
        arah = "x"
        for _ in range(600): # worst case
            count += 1
            if count >= 400 and (x <= 0 or x >= 1500 or y <= 0 or y >= 1500): break # best case kurang dari worst case
    ...
    def generate_buildings():
        ...
        for _ in range(4):
            while True:
                x, y = random.randint(0, size - 5), random.randint(0, size - 3)
                if is_valid_position(x, y, 5, 3):
        ...
        for _ in range(10):
            while True:
                ...
                if is_valid_position(x, y, 2, 2):
                    place_building(small_building_image, x, y, 2, 2)
                    break

        for _ in range(10):
            while True:
                x, y = random.randint(0, size - 1), random.randint(0, size - 2)
                if is_valid_position(x, y, 1, 2):
                    place_building(house_image, x, y, 1, 2)
                    break

        for _ in range(10):
            while True:
                x, y = random.randint(0, size - 10), random.randint(0, size - 5)
                if is_valid_position(x, y, 10, 5):
                    place_building(big_building_image, x, y, 10, 5)
                    break

    def generate_green_areas():
        for x in range(size):
            for y in range(size):
