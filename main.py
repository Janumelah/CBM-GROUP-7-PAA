...
big_building_image = Image.open("assets/big_building.png").convert("RGBA")
medium_building_image = Image.open("assets/medium_building.png").convert("RGBA")
small_building_image = Image.open("assets/small_building.png").convert("RGBA")
house_image = Image.open("assets/house.png").convert("RGBA")
bush_image = Image.open("assets/bush.png").convert("RGBA")  # Load the bush image
cross_road = Image.open("assets/+_road.png").convert("RGBA") 
horizontal_road = Image.open("assets/horizontal_road.png").convert("RGBA") 
vertical_road = Image.open("assets/vertical_road.png").convert("RGBA") 
t_down = Image.open("assets/troad_turn_down.png").convert("RGBA")
t_up = Image.open("assets/troad_turn_up.png").convert("RGBA")
t_left = Image.open("assets/troad_turn_left.png").convert("RGBA")
t_right = Image.open("assets/troad_turn_right.png").convert("RGBA")
left_up = Image.open("assets/down_turn_left.png").convert("RGBA")
left_down = Image.open("assets/turn_left.png").convert("RGBA")
right_down = Image.open("assets/turn_right.png").convert("RGBA")
right_up = Image.open("assets/down_turn_right.png").convert("RGBA")
...
resized_bush = bush_image.resize((1 * map_scale, 1 * map_scale))  # Resize the bush image to 10x10 cells
...
def generate_map(cell_size, map_scale):
    ...
    road_cells = set()
    building_cells = set()
    bush_cells = set()  # Keep track of bush locations
    vertice = []
    count = 0
    ...
    def generate_roads():
        ...
        for _ in range(2000):
            ...
            if 0 <= x < 1500 and 0 <= y < 1500:
                ...
                # Tentukan gambar jalan yang akan digunakan
                road_image = resized_horizontal_road if arah == "x" else resized_vertical_road
                mask = road_image.split()[3]
                # Menggambar gambar jalan sepanjang jalan yang dihasilkan
                if arah == "x":
                ...
                for cell in road_cells:
                    ...
                    road_image = None
                    mask = None
                    # Tentukan gambar jalan yang akan digunakan berdasarkan kondisi
                    ...
                    # Jika ada gambar jalan yang dipilih, paste ke posisi yang sesuai di canvas
                    if road_image:
                        mask = road_image.split()[3]
            ...
            for _ in range(num):
                ...
                while not placed and attempts < 250:
                    ...
                    if can_place_building(x, y, width, height) and is_adjacent_to_road(x, y, width, height):
                        ...
                        if can_place_building(x, y, new_width, new_height):  # Re-check after rotation
